...
public class QuerySnapshotsStreamHandler implements StreamHandler {
  ...
  public QuerySnapshotsStreamHandler(Query query, Boolean includeMetadataChanges,
      DocumentSnapshot.ServerTimestampBehavior serverTimestampBehavior, ListenSource source,
      Executor snapshotExecutor) {
    this(query, includeMetadataChanges, serverTimestampBehavior, source, snapshotExecutor,
        mainThreadExecutor());
  }

  QuerySnapshotsStreamHandler(Query query, Boolean includeMetadataChanges,
      DocumentSnapshot.ServerTimestampBehavior serverTimestampBehavior, ListenSource source,
      Executor snapshotExecutor, Executor eventExecutor) {
    this.query = query;
  ...
  private void emitNextSnapshot() {
    ...
    try {
      ...
      postSuccess(QuerySnapshotChunkProtocol.start(emission.id, emission.documentCount,
                      emission.documentChangeCount, emission.metadata),
    ...
    try {
      if (emission.documents.hasNext()) {
        List<GeneratedAndroidFirebaseFirestore.InternalDocumentSnapshot> chunk =
            emission.documents.nextChunk(values
        ...
        postSuccess(QuerySnapshotChunkProtocol.itemChunk(
      ...
      if (emission.documentChanges.hasNext()) {
        List<GeneratedAndroidFirebaseFirestore.InternalDocumentChange> chunk =
            emission.documentChanges.nextChunk(values
                -> QuerySnapshotChunkProtocol.encodedEnvelopeSize(
                    QuerySnapshotChunkProtocol.itemChunk(
                        emission.id, QuerySnapshotChunkProtocol.DOCUMENT_CHANGES_KIND, values)));
        postSuccess(QuerySnapshotChunkProtocol.itemChunk(
  ...
  private final class SnapshotEmission {
    ...
    final QuerySnapshotChunker<DocumentSnapshot,
        GeneratedAndroidFirebaseFirestore.InternalDocumentSnapshot> documents;
    final QuerySnapshotChunker<DocumentChange,
        GeneratedAndroidFirebaseFirestore.InternalDocumentChange> documentChanges;
    ...
    SnapshotEmission(long id, QuerySnapshot snapshot) {
      ...
      documents = new QuerySnapshotChunker<>(nativeDocuments.iterator(),
          value
          -> PigeonParser.toPigeonDocumentSnapshot(value, serverTimestampBehavior),
          QuerySnapshotChunkProtocol::encodedMessageSize,
          QuerySnapshotChunkProtocol.MAX_ENCODED_ENVELOPE_BYTES);
      documentChanges = new QuerySnapshotChunker<>(nativeDocumentChanges.iterator(),
          value
          -> PigeonParser.toPigeonDocumentChange(value, serverTimestampBehavior),
          QuerySnapshotChunkProtocol::encodedMessageSize,
          QuerySnapshotChunkProtocol.MAX_ENCODED_ENVELOPE_BYTES);
    }
  }
